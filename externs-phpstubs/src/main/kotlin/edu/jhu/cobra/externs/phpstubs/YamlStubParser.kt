...
object YamlStubParser {
    ...
    fun parse(
        ...
        val records = entries.map { entry -> parseEntry(entry, extension) }
        return ParseResult(
    ...
    private fun parseEntry(
        ...
        return when (tag) {
            "function" -> parseFunction(map, extension)
            "method" -> parseMethod(map, extension)
            "class" -> parseClass(map, extension)
            "constant" -> parseConstant(map, extension)
            "class_constant" -> parseClassConstant(map, extension)
            "property" -> parseProperty(map, extension)
    ...
    private fun parseClassConstant(
        map: Map<*, *>,
        extension: String,
